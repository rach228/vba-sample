...
First, download the vulnerability report for the target cluster from Red Hat Advanced Cluster Security or from the scheduled email reports. Convert the vulnerability report from CSV format to XLSM format to enable macros. Copy the VBA macro code from `create_pivot_table.vba`. Take note that the namespace filter list in the VBA code is a sample. You may refer to `create_pivot_table_updated.vba` which contains default list of OpenShift platform-related namespaces.
